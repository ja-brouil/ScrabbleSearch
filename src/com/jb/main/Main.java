...
public class Main {
	...
	public static void main(String[] args) throws IOException, ParseException {
		...
		// Create Dictionnary
		JSONObject dictionnaryJsonObject = WordListFactory.getJsonList("https://raw.githubusercontent.com/adambom/dictionary/master/dictionary.json");
		...
		for (@SuppressWarnings("rawtypes") // Yes this is a raw type but for simplicity's sake, this will still work
		Iterator iterator = dictionnaryJsonObject.keySet().iterator(); iterator.hasNext();) {
	...
	private static boolean isWordDoable(char[] letters, String word) {
		LinkedList<Character> inputCharacters = new LinkedList<>();
		for (int i = 0; i < letters.length; i++) {
			inputCharacters.add(letters[i]);
		}
		 
		LinkedList<Character> wordCharacters = new LinkedList<>();
		...
		// If we still have letters in the word we are checking, we cannot make this word
